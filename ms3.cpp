...
int main(int argc, char** argv)
{
	...
	for (const auto& o : g_incomplete) o.display(cout);
	...
	// cleanup
	for (auto station : theStations) delete station;
...
static void loadFromFile(const char* filename, vector<T>& theCollection)
{
	...
	if (!filename) throw string("ERROR: No filename provided.");
	
	// VARIABLE DECLARATION.
	ifstream file(filename);
	string record;

	if (!file) throw string("Unable to open [") + filename + "] file.";
